...
/**
 * 阅读思路：
 * 1- 按功能模块阅读，以功能驱动阅读，搜索关键词串联起来阅读 -- 这轮阅读要搞明白什么
 * 2- 阅读次序：开头/结尾/中间
 * 3- 往复/轮式/咀嚼式阅读 -- 一轮一轮阅读
 ...
 * 11- 最后希望你能画出这个原型链的链式关系 --  多尝试画几次，不断修正
 * 12- 不要怕尝试 -- 从某种意义上说，你既一无所有，也身不由己，无法成全自己，以上，概括就是：你别无选择
 * 13- 临摹一遍 + 手写一遍mini版 + 出blog
 * 14- 先不要看老师的注释 -- 自己先读一遍，去理解 -- 然后去看注释等
 * 15- 不要怕查阅资料 
 */
...
var Zepto = (function() {
  ...
  // me: 变量的定义尽量放在代码的首部
  // me: 这里的$是用来内部调用自己的方法的别名，外部无法访问，设计的一个特点就是自产自销 -- 自己给自己造轮子，用自己的轮子造汽车,也就是自己利用自己编织的进化的梯子套梯子登上金字塔
  // me: 下面这些变量-也不是一开始就从设计层面就定义到了-- 在出生发展的过程中 -- 在需要中逐步增加的 -- 在迭代中增殖设计的
  var undefined, key, $, classList, 

      // 获取数组的slice 和 filter（返回数组中的满足回调函数中指定的条件的元素）方法
      // me: 这里的emptyArray代表Array向后面提供Array的一些方法（继承）
  ...
  $.map = function(elements, callback){
    ...
    // me: push到数组才是map中callBack不断返回(利用for实现map)的真相/关键
    // 数组，或者对象数组
  ...
  /*------------------------------------------------------（三）在一个对象中封装zeptob暴露出去的api-----------------------------------------------------------*/
  /**
   * me：所有一个对象的方法都放在它的原型中 -- 既prototype中
   */
  ...
  $.fn = {
    ...
    /*=====================（1）me:继承一些已有的方法,暴露为方法=========================== */
    // 为何要这么多数组的方法？
    // 因为一个 zepto 对象，本身就是一个数组
    // me: 这里就是继承了一部分Array的方法
    ...
    /*=====================（2）me:我们看到-对外的API其实是对挂载在$上的一些基础方法的封装（对输入/输出的适配/包装）--其实也是一种分层设计的思想=========================== */
    // me:一些基础方法其实就是把很多API中的复用的业务抽象/设计/封装出来
    // `map` and `slice` in the jQuery API work differently
    // from their array counterparts
    map: function(fn){
      // $.map 上文定义的， $.map = function (elements, callback) {....}
      // $.map 作用：针对 elementcous（对象、对象数组或数组），对每个元素都经过 callback 函数的过滤，并将过滤通过的元素，push到一个新数组中，返回新数组

      // 最后，用 $ 封装返回
      return $(
        // $.map 返回的是一个数组
        // me:这里的this就是代表传入的元素/或者调用map的对象
        $.map(this, 
          // 针对每一个元素，都执行传入的函数，如果函数返回的 !=null 就将插入到新返回的数组
          // me: 这里使用匿名函数是为了进行函数定义，回调fn的定义是在函数外部，所以，这里定义一个匿名函数把它包起来
  ...
  zepto.uniq = uniq
  // me:deserialize 序列化 -- 就是把字符串变成各种值， ‘12’ --> 12, 'false' --> false
...
// 把定义的zepto挂到window对象上
window.Zepto = Zepto
// 这样的话，zepto除了自己以外，还可以有个别名：$ 这样用户可以用$享受到zepto打包提供的这些方法
