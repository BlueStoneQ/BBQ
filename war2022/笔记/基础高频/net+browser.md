...
2. token:JWT:
   ...
   - head中一般是一些元信息：例如加密算法等
   - payload: 用户的登录信息 + 过期时间；所以 一般服务端难以主动让token失效
   - signature: 由于token是服务端生成的 所以
  ...
  - 其实整个https通信过程主要分为2部分：
    1. 生成相同的对话秘钥 
      ...
      - 具体过程：
        ...
        - server: 收到请求后，生成一个随机数server_num2 + 数字证书(+ public_key)
...
- 防御：
  - 服务端：
    - 尽量使用post
    - 表单提交 加入验证码 - 确保表单提交是一个用户行为 而不是黑客行为
## 网络模型
- 应用层
  - 应用层
