...
(function () {
    ...
    Sampling = class {
        constructor(pageInstance = wx) {
            ...
            _pageInstance = pageInstance; // 这个变量 利用闭包 共享页面实例给静态方法
    ...
    Sampling.getCurrentPageById = function (pageId) {
        ...
        // 当 组件所在页面的pageId和当前页面的pageId一致的时候 可以确定当前页面就是组件所在的页面 其实就是加了一层防御和保险
        return pageId && _pageInstance.getPageId() === pageId ? _pageInstance : null;
    } 
})()
