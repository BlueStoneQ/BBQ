...
class HistoryRouter {
  ...
  push (path) {
    ...
    // 因为pushState不会引起popstate事件 所以 这里需要手动调用handle https://developer.mozilla.org/zh-CN/docs/Web/API/Window/popstate_event
    this._handle({ state: { path } });
  }

  replace (path) {
    history.replaceState({ path }, null, path);
    // 因为replaceState不会引起popstate事件 所以 这里需要手动调用handle https://developer.mozilla.org/zh-CN/docs/Web/API/Window/popstate_event
    this._handle({ state: { path } });
  }
