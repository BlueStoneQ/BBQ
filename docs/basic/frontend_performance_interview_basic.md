...
>
> 📎 参考资料：[H5前端面试百科 - 性能优化篇](https://interview.html5.wiki/performance.html)
...
| 层 | 手段 | 关键词 |
|----|------|--------|
| 网络层 | DNS 预解析、TCP 预连接、HTTP/2 多路复用、CDN 就近访问 | `dns-prefetch` `preconnect` `CDN` |
| 请求层 | 减少请求数（合并/雪碧图/内联）、减小体积（Gzip/Brotli）、接口聚合 | `Gzip` `合并` `聚合` |
| 资源层 | 包体优化（Tree-shaking/代码分割/按需加载）、图片优化（WebP/压缩/响应式） | `SplitChunks` `lazy import` `WebP` |
| 缓存层 | 强缓存（Cache-Control）、协商缓存（ETag）、Service Worker、localStorage | `max-age` `ETag` `304` |

### 2. 渲染性能（首屏可见时间 — 资源到了但还没画出来）

| 层 | 手段 | 关键词 |
|----|------|--------|
| 关键渲染路径 | CSS 放 head、JS 放底部或 async/defer、减少阻塞 | `async` `defer` `关键CSS内联` |
| 首屏策略 | SSR 服务端渲染、预渲染、骨架屏、渐进式加载 | `renderToString` `skeleton` |
| 懒加载 | 图片懒加载（Intersection Observer）、路由懒加载、组件懒加载 | `data-src` `React.lazy` `动态import` |

### 3. 运行时性能（交互流畅度 — 页面已经出来了，操作卡不卡）

| 层 | 手段 | 关键词 |
|----|------|--------|
| DOM 操作 | 批量更新、DocumentFragment、虚拟 DOM、虚拟列表 | `Fragment` `react-window` |
| 回流与重绘 | 用 transform 代替 top/left、批量改 class、DOM 离线、避免读写交替 | `transform` `will-change` `合成层` |
| 事件优化 | 节流（throttle）、防抖（debounce）、事件委托 | `throttle` `debounce` `delegation` |
| 长任务拆分 | requestIdleCallback、Web Worker、时间切片 | `rIC` `Worker` `Fiber` |
| 动画 | requestAnimationFrame、GPU 加速（transform/opacity）、独立图层 | `rAF` `translate3d` `will-change` |

### 4. 感知性能（用户体感 — 实际没变快但感觉快了）

| 层 | 手段 | 关键词 |
|----|------|--------|
| 加载态 | 骨架屏、Loading 动画、渐进式图片（模糊→清晰） | `skeleton` `blur-up` |
| 交互反馈 | 乐观更新、按钮 loading 态、过渡动画 | `optimistic update` |
| 指标 | FCP < 1.8s、LCP < 2.5s、TTI、CLS < 0.1、INP < 200ms | `Web Vitals` `Lighthouse` |

### 5. 构建性能（开发体验 — 打包快不快）

| 层 | 手段 | 关键词 |
|----|------|--------|
| 缩小范围 | include/exclude、resolve.modules、alias | `babel-loader exclude` |
| 缓存 | 持久化缓存（Webpack5 filesystem）、babel cacheDirectory | `cache: filesystem` |
| 并行 | thread-loader、多进程压缩（TerserPlugin parallel） | `thread-loader` `parallel` |
| 分析 | webpack-bundle-analyzer、speed-measure-plugin | `BundleAnalyzer` |
