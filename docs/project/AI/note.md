...
Harness 原意"马具"，Agent Harness 指 Agent 的运行外壳——套在 Agent 外面的控制层。作用：控制 Agent 能做什么（权限边界）、观察在做什么（可观测性）、评估做得好不好（评估）、出问题时拉住它（安全护栏）。

组成：权限控制（工具白名单）+ 可观测性（日志/Trace/指标）+ 安全护栏（超时/限额）+ 评估框架（准确性/相关性）。

实际产品中的体现：Kiro 的 preToolUse Hook 和 Supervised 模式、Claude Code 的 allowedTools 白名单、OpenClaw 的 Steering Monitor 和 Workspace 限制、小象的 /stop /undo 和 Shell 白名单。

回扣第一性原理：Harness 优化的是 Agent 循环控制——不是让 Agent 跑得更快，而是更安全、更可控、更可观测。一句话：Harness 是 Agent 的安全带和仪表盘。


### Q: RAG 中的向量编码是查表吗？向量是矩阵还是数字？

**向量是什么**：一维数组（一组有序浮点数），比如 `[0.12, -0.34, 0.56, ...]` 共 1536 个数字。不是矩阵（矩阵是二维的）。

**LLM 内部 Embedding vs RAG Embedding 的区别**：
- LLM 内部：单个 Token → 查 Embedding 表 → 一个向量。是查表操作。
- RAG：一整段文本 → 跑 Embedding 模型（一个专门的小型神经网络）→ 输出一个向量。不是查表，是跑模型推理。

**RAG 检索过程**：
1. 知识库文档切成小段 → 每段跑 Embedding 模型 → 得到向量 → 存入向量数据库
2. 用户提问 → 跑同一个 Embedding 模型 → 得到问题向量
3. 在向量数据库里找距离最近的文档段落向量 → 取出原始文本 → 塞进 Prompt

向量代表文本的语义，语义相近的文本向量距离近。
