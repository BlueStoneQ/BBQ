...
**本质**：一种动态扩充 LLM 上下文的范式。LLM 知识是固定的（训练截止日期），上下文窗口是有限的，RAG 解决的是"怎么在有限窗口里塞进最相关的外部信息"。

**流程**：
```
用户提问 → [R] Retrieval 检索相关文档 → [A] Augmented 塞进 prompt 增强上下文 → [G] Generation LLM 生成回答
```

- **R**（Retrieval）：从外部数据源检索相关内容。手段很多——BM25 关键词检索、向量检索、知识图谱、SQL 查库、搜索引擎，都算
- **A**（Augmented）：用检索到的内容增强 prompt，让 LLM 能"看到"它本来不知道的信息
- **G**（Generation）：LLM 基于增强后的上下文生成回答（有据可依，减少幻觉）

**Mako 的上下文管道（L1-L5 分层）本质上就是一套 RAG 系统**，只不过检索源不是外部知识库，而是对话历史和项目文件。下面的 BM25 就是 L5 归档检索层用的检索算法。
...
> BM25 是算法（定义怎么算相关性分数），MiniSearch 是实现它的纯 JS 库（几 KB）。关系类似：快速排序是算法，`Array.sort()` 是实现。
