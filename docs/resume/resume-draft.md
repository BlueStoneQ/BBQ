...
10 年前端经验，具备跨端开发与框架设计经验的大前端架构师。核心能力：全栈全链路交付 + 跨端开发与框架设计 + 前端工程化体系建设 + AI Agent 开发与 AI 融入开发实践。五端实战覆盖（Web / RN App / 小程序 / 桌面端 Electron / 跨端框架层）。
...
面向前端主流源码类型（xml, css, js），基于 AOT 以特殊注释作为编译指令进行条件编译，实现对源码内容根据不同目标平台(例如跨端方案)进行精确裁剪，减少打包体积，减轻包体和运行时压力。
...
#### ① 负载性能分析平台（全栈 · 全链路开发）【能力线 1 + 3】

- 背景：对设备/芯片的负载（指令数）、功耗、性能、流畅度（帧率）等进行可视化分析优化的平台搭建开发
- 全链路开发（**从 0 到 1 独立交付**）：
  ...
  - 线上问题排查：日志分析、异常定位、性能瓶颈排查优化(大文件上传优化等)

#### ② 快应用框架（跨端框架 · Android 原生）【能力线 2 + 3】

- 系统级快应用运行时框架，类 RN 架构（**JS 驱动 Native View 渲染，非 WebView**），**V8 引擎 + J2V8 同步 Bridge**
- 包体优化：使用条件编译、resConfigs、shrinkResources、R8 等手段，预装包 **153MB → ~60MB**，dex **44.4MB → 27MB（-39%）**
...
**一键注册**
- RN 混合原生开发，一键获取用户手机号（Android/iOS 原生能力桥接）
...
- 代码规范设计
- 推动**全链路（编码时Lint → Git Hooks → CI/CD）卡控方案**完善
- Review 机制的设计和推动落地
...
主 R，负责调研收集问题、设计解决方案，服务**多个业务方向**统一接入：
...
- 面向政企 toB 领域，业务类型：PC 端（门户网站、管理平台）、移动端（H5/HybridApp/微信小程序）、大屏、直播监控
- 技术栈：React、MobX/Redux、Ant Design、ECharts、jQuery、Bootstrap
- 带队开发：带领 5 人团队完成天柱山景区多个信息平台的前后端开发与交付
