...
### 跨端开发与框架设计
- [Flutter](../../interview/LN/flutter/README.md)：Dart/Widget、Skia渲染管线、Riverpod、Platform Channel/FFI、混合栈、引擎预热
- Android 原生：Kotlin/Java、Gradle、NDK/JNI、Native Shell容器设计
- iOS 原生：Swift/OC、CocoaPods、XCFramework、混编桥接、容器生命周期管理
- C++：跨平台渲染框架、[Yoga布局引擎](../../interview/LN/quickapp-platform/layout-engine-yoga.md)、[QuickJS/V8嵌入](../../interview/LN/quickapp-platform/js-engine-capabilities.md)、[跨层Bridge设计（FFI/JNI/J2V8）](../../interview/LN/rust-flutter-card/cross-layer-communication.md)、SO/dylib编译、LVGL渲染
- 桌面端：Flutter Desktop / Electron, 快应用配套IDE设计开发
- [动态渲染框架(Server-Driven UI)](../../interview/LN/rust-flutter-card/README.md): JSON/XML协议驱动动态渲染、diff增量渲染、事件交互
...
### [Mako](../../project/AI/mako/README.md) — AI Coding Agent
...
- [分层架构](../../interview/LN/rust-flutter-card/architecture-overview.md): — 应用层(卡片 DSL) → 运行时层(模板解析 + 响应式) → 引擎核心层(C++ DOM + CSS + Yoga Layout) → 渲染后端层(Flutter/Android)
- [渲染管线](../../interview/LN/rust-flutter-card/input-to-render-pipeline.md)：DOM Diff → Layout → RenderTree → 渲染后端（Flutter/LVGL/Android）
- [跨层通信](../../interview/LN/rust-flutter-card/cross-layer-communication.md)：Dart FFI 直调 C++ .so / Platform Channel 桥接 Native / JNI·OC++ 调用引擎层
...
#### ② [快应用框架](../../interview/LN/quickapp-platform/README.md)（类 RN 跨端框架 · Android 原生）
系统级快应用运行时，类Vue DSL(JS)驱动 Native View 渲染（非 WebView），V8 + [J2V8 同步 Bridge（类 JSI）](../../interview/LN/quickapp-platform/bridge-communication-deep-dive.md)。     
...
基于XC Flutter混合框架，负责机酒模块业务开发 → [XC Flutter 方案参考](../../interview/LN/ref/CFlutter/reference.md)
...
- 参与集团 [MTFlutter](../../interview/LN/ref/MTFlutter/reference.md)试点接入，负责司机端部分低频业务页面Flutter化
