...
> 简历写了"DEX 布局优化，热代码前置减少 page fault，PSS MAX 41MB → 35.8MB"

### 需要补全

- [ ] 我们实际做了什么？具体操作步骤是什么？
- [ ] 用的是 Baseline Profile 还是其他方式？
- [ ] 怎么确定哪些是"热代码"？（Perfetto trace？ART profiling？手动指定？）
- [ ] 优化前后的完整数据对比（不只是 PSS，还有启动时间、page fault 数量）
- [ ] Android 常见内存优化方案体系化梳理：
  - [ ] DEX 布局优化（热代码前置）
  - [ ] R8 裁剪（减少 dex 体积 → 减少 mmap 常驻页）
  - [ ] 图片降采样 / Bitmap 复用
  - [ ] 对象池（减少 GC 压力）
  - [ ] LeakCanary 泄漏检测
  - [ ] onTrimMemory 主动释放
  - [ ] 大对象延迟初始化
  - [ ] Native 内存管理（V8 Heap 控制）
