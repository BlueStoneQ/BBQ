...
### 大文件上传 开始没有走MPU的时候 是不是会触发什么告警或者指标异常

会。整个文件一次性 PUT 时，Node 进程出现以下异常：

| 指标 | 异常表现 | 原因 |
|------|---------|------|
| **RSS / Heap Used** | 飙升（4.4G 文件 = 内存中 buffer 4.4G+） | 整个文件读进内存再上传 |
| **Event Loop Lag** | 飙高（秒级） | 大 Buffer 操作阻塞主线程 / GC 暂停 |
| **P95 响应时间** | 飙升（其他请求排队等） | 单线程被占着，后续请求全等 |
| **OOM Killed** | Pod 被 K8s 杀 | 内存超 container limit |

**叙事串联**：Grafana 看到上传接口 P95 飙到 120s+，同时 RSS 接近 Pod 内存上限触发告警。排查发现大文件（4.4G）走的是整体 PUT。方案：改 S3 MPU 分片并发上传（presigned URL 直传，浏览器 6 路并发，每片 64MB）。优化后 121s → 42s，内存稳定。

**并发数 6 怎么来的**：前端 presigned URL 直传 S3，浏览器对同域 HTTP/1.1 并发上限就是 6——客户端硬限制。实测对比的是分片大小（64MB vs 128MB vs 32MB），64MB 在单片上传耗时 + 重传成本之间最优。