...
- [6.8 为什么不用 LangChain？桌面端该用吗？](#68-为什么不用-langchain桌面端该用吗)
...
| 平台 | 是否必须签名 | 不签名后果 | 证书来源 | 费用 | 签名工具 |
|------|:---:|------|------|------|------|
| macOS | 必须 | 无法打开（系统直接拦截） | Apple Developer Program | $99/年 | codesign + Notarization |
| Windows | 建议 | SmartScreen 蓝色警告"未知发布者" | CA 机构（DigiCert/Sectigo） | $200-400/年 | signtool.exe |
| Linux | 不需要 | 无影响 | — | — | — |

Windows 贵是因为证书由第三方 CA 签发（不是微软自己卖），EV 证书还要物理 USB Key。macOS 便宜是因为 Apple 自己卖开发者账号，证书免费附带。

**CI 中怎么用**：证书文件（.pfx / .p12）base64 编码后存到 CI 的 secrets 环境变量，构建时解码写入临时文件，electron-builder 自动读取签名。不把证书提交到 Git。
...
**最佳实践**：`SafeAreaProvider` 包在 App 最外层（只包一次，提供 insets 上下文），`SafeAreaView` 按需包在每个需要避让的页面外面（不是所有页面都需要——全屏图片预览就不包）。
...
| 第一次 Response 带 | 第二次 Request 带 | 服务器判断 |
|-------------|-------------|-----------|
